...
void ImageProcessing1(){
	...
	player1.setLvl(0);
	player2.setLvl(0);
	VideoCapture stream1(1); //0 is the id of video device.0 if you have only one
	...
	while (true) {
		...
		int p1att = combiner.CalculateMunchkinAttack(tempGearP1, tempGear1P1, tempMunchkinP1, tempMunchkinP2);
		int p1lvl = player1.getLvl();
		int p1_total = p1lvl + p1att;
		player1.setAttack(p1_total);
...
int main(int argc, char *argv[])
{
	...
	int p1Attack = 2;
	int p1Lvl = 7;
	thread t1(ImageProcessing1);
	QApplication a(argc, argv);
	QtGuiApplication3 w;
	w.p1Attack = p1Attack;
	w.p1Lvl = p1Lvl;
	
	w.monsterPlayed = monsterPlayed;
	w.turn = turn;
	w.show();
	//w.UpdateP1Lvl(p1Lvl);
	return a.exec();
